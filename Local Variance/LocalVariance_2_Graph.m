...
normMat1 = zscore(origMat1(:,2:end));
normMat2 = zscore(origMat2(:,2:end));
normMat3 = zscore(origMat3(:,2:end));
normMat4 = zscore(origMat4(:,2:end));
...
filename = 'Local_Variance_2.xlsx';
for type = 1:4
    ...
    for i  = 1:n
        [ct, local_var] = local_variance_2(normMat{type}(i,1) , long_lat_mat, normMat{type});
        loc_var_mat = vertcat(loc_var_mat, [ct, local_var]);
    ...
    for j = 1:n
        ...
        %weight
        [long, lat, loc_var] = get_long_lat_weight(ct,1,loc_var_mat, long_lat_mat);
        x = horzcat(x,long);
        y = horzcat(y,lat);
        z = horzcat(z,loc_var);
    end
    ...
    %from the data
     ...
     switch type
         case 1
             title('Local Variance 2:Original')
         case 2
             title('Local Variance 2:Car/Van/Campers')
         case 3
             title('Local Variance 2:Tents/Encampments')
         case 4
             title('Local Variance 2:Shelter')
     end
     
    %Local Variance table
    loc_var_tab = array2table(loc_var_mat,'VariableNames',{'census_track', 'local_variance'});
    ...
    loc_var_tab_max = array2table(loc_var_mat_max,...
         'VariableNames',{'census_track', 'local_variance'});
    loc_var_tab_min = array2table(loc_var_mat_min,...
         'VariableNames',{'census_track', 'local_variance'});
    
    %write to excel
    xlswrite(filename, {...
        'Top 10 census tracks with maximum local variance'...
        },type,'A1'); 
    writetable(loc_var_tab_max(1:10,:),filename,'Sheet',type,'Range','A2')

    xlswrite(filename, {...
        'Top 10 census tracks with the minimum local variance'...
        },type,'A14'); 
    writetable(loc_var_tab_min(1:10,:),filename,'Sheet',type,'Range','A15')

    xlswrite(filename, {...
    'Top 10 census tracks with the highest homeless population density'...
    },type,'E1'); 
    writetable(homelesspoptab(1:10,1),filename,'Sheet',type,'Range','E2')
